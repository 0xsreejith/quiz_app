...
class HomeController extends FullLifeCycleController with FullLifeCycleMixin {
  ...
  // Generation counter: each new load increments this. A load only applies
  // its results if its captured generation still matches the current one.
  // This prevents a slow cache-based loadData() from overwriting a faster
  // server-based refreshAfterQuiz() that ran concurrently.
  int _loadGen = 0;

  // Timestamp of the last successful data load — used to debounce onResumed.
  ...
  Future<void> loadData({bool forceServer = false}) async {
    // Increment and capture generation. If a newer load starts while this
    // one is in-flight, the captured gen will no longer match _loadGen and
    // this load will discard its results.
    final int gen = ++_loadGen;
    isLoading.value = true;
    errorMessage.value = '';
    try {
      await Future.wait<void>([
        _loadTopPerformers(forceServer: forceServer, gen: gen),
        _loadUserStats(forceServer: forceServer, gen: gen),
      ]);
  ...
  /// Called from ResultPage after quiz completion.
  /// Uses forceServer so Firestore cache is bypassed and fresh data is shown.
  Future<void> refreshAfterQuiz() async {
    ...
    try {
      ...
      // Best effort — do not show error on refresh failure.
    } finally {
      if (gen == _loadGen) {
        isLoading.value = false;
        _lastLoadTime = DateTime.now();
  ...
  Future<void> _loadTopPerformers({bool forceServer = false, int? gen}) async {
  ...
  Future<void> _loadUserStats({bool forceServer = false, int? gen}) async {
    final String? uid = _authService.currentUser?.uid;
    if (uid == null) return;
